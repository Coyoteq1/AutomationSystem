...
{
    private static readonly HashSet<PrefabGUID> MagicSourceGuids = new()
    {
    ...
    static void WeaponLevelPatch(WeaponLevelSystem_Spawn __instance)
    {
        ...
        try
        {
            foreach (Entity entity in entities)
            {
                ...
                {
                    var playerCharacter =
                        CrowbaneArenaCore.EntityManager.GetComponentData<PlayerCharacter>(entityOwner.Owner);
                    // Record level system not implemented - placeholder
                    Plugin.Logger?.LogInfo(
                        $"Weapon level update for player: {CrowbaneArenaCore.EntityManager.GetComponentData<User>(playerCharacter.UserEntity).CharacterName}");
                }
            }
        }
        finally
        ...
        try
        {
            foreach (Entity entity in entities)
            {
                ...
                if (CrowbaneArenaCore.EntityManager.HasComponent<PlayerCharacter>(entityOwner.Owner))
                {
                    ...
                    // Record level system not implemented - placeholder
                    Plugin.Logger?.LogInfo(
                        $"Armor level update for player: {CrowbaneArenaCore.EntityManager.GetComponentData<User>(playerCharacter.UserEntity).CharacterName}");
                }
            }
        }
        finally
    ...
    private static void BuffDebugPatch(BuffDebugSystem __instance)
    {
        ...
        var entities = CrowbaneArenaCore.EntityManager.CreateEntityQuery(ComponentType.ReadOnly<PrefabGUID>())
