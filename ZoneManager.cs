...
namespace CrowbaneArena
{
    ...
    {
        public static float3 ArenaCenter { get; set; }
        ...
        public static float3 SpawnPoint { get; set; }
        ...
        public static void CheckPlayerZones(Entity playerEntity, float3 position)
        {
            ...
            if (inEntry && !wasInArena)
            {
                EnterArena(playerEntity);
            }
            else if (inExit && wasInArena)
            {
                ExitArena(playerEntity);
            }
            else if (inArenaTerritory && !wasInArena)
            {
                EnterArena(playerEntity);
            }
            else if (!inArenaTerritory && wasInArena)
        ...
        private static bool IsValidEntity(Entity entity)
        {
            ...
            try
            {
                ...
                if (em.HasComponent<PlayerCharacter>(entity))
                {
                    ...
                    Plugin.Logger?.LogInfo($"PlayerCharacter found for entity {entity}, UserEntity: {playerCharacter.UserEntity}");
                }
                else
                {
                    Plugin.Logger?.LogWarning($"No PlayerCharacter component found on entity {entity}");
                }
        ...
        public static void ManualEnterArena(Entity playerEntity)
        {
            ...
            try
            {
                ...
                {
                    Plugin.Logger?.LogError($"[ManualEnterArena] Error unlocking VBloods for {playerName}: {ex.Message}");
                    Plugin.Logger?.LogError(ex.StackTrace);
                ...
                try 
                {
                    Plugin.Logger?.LogInfo($"[ManualEnterArena] Granting abilities to {playerName}...");
                    AbilityManager.GrantAllAbilities(playerEntity);
                    Plugin.Logger?.LogInfo($"[ManualEnterArena] Successfully granted abilities to {playerName}");
                }
                catch (Exception ex)
                {
                    Plugin.Logger?.LogError($"[ManualEnterArena] Error granting abilities to {playerName}: {ex.Message}");
                    Plugin.Logger?.LogError(ex.StackTrace);
                    // Continue even if ability granting fails
                }
